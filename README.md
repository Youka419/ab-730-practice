...
アプリ右上の **ヘルプ** に手順があります。初回は **新規登録**（ID・パスワード・秘密の質問）が必要です。履歴はログイン中の人ごとに分かれます。

## 含まれるもの

| ファイル | 内容 |
|---|---|
| `index.html` | ログイン・出題・採点・履歴・ヘルプ・公式模擬への入口 |
| `questions.js` / `questions-extra.js` | 基礎知識編 150 問（3 領域 × 50 問。中間レベル。公式 Study Guide 2026年7月22日時点） |

出題順と選択肢は毎回シャッフルされます。正解率・誤答リストは、開いているブラウザの localStorage に **ID ごと** に保存されます（GitHub には同期されません。メールでのパスワード再設定はありません）。公式模擬の成績は Microsoft Learn 側です。
